...
class Page_pelayanan_publik extends MY_Controller
{
    private $base           = 'admin';
    private $menu           = 'pelayanan_publik';
    private $table          = 'page_pelayanan_publik';
    ...
    public function get_data()
    {
        ...
        foreach ($list_data as $key => $value) {
            ...
            if ($value['image'] != '' && $value['image'] != null) {
                $img        = base_url('uploads/page_pelayanan_publik/' . $value['image'] . '');
            } else {
                $img        = '';
            }
            $row['image']   = $img;
    ...
    public function prosesTambah()
    {
        if ($_FILES['image'] != '') {
            ...
            $config['upload_path']      = 'uploads/page_pelayanan_publik';
    ...
    public function prosesUbah()
    {
        ...
        $config = array(
            'encrypt_name'  => TRUE,
            'upload_path'   => "uploads/page_pelayanan_publik",
        ...
        if ($_FILES['image'] != '') {
            if (!$this->upload->do_upload('image')) {
                if ($kosongkan_image == "1") {
                    if ($data_ppid['image'] != '') {
                        unlink('./uploads/page_pelayanan_publik/' . $data_ppid['image']);
                    }
                    $file_name = "";
                } else {
                ...
                $file_name = $this->upload->data('file_name');
                $this->page->_create_thumbs('page_pelayanan_publik', $file_name);
            }
        } else {
            $file_name = '';
        }
