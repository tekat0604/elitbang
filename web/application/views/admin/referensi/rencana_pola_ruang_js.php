...
function daftar_st(){
    ...
    $.ajax({
        type  : 'ajax',
        url   : '<?php echo base_url()?>admin/referensi/daftar_rencana_pola_ruang',
        async : false,
        dataType : 'json',
        success : function(data){
            ...
            for(i=0; i<data.length; i++){
                html += '<tr>'+
                        ...
                        '<td style="text-align:center;">'+data[i].nama_st+'</td>'+
                        '<td style="text-align:center;">'+
                            '<button type="button" class="btn btn-sm btn-warning mb-10 item_edit" data="'+data[i].id_st+'"><i class="fa fa-edit"></i> Edit</button> '+
