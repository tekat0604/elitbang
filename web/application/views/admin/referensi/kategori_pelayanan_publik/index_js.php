<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		...
		$(document).on('click', '#konfirmasi_hapus', function(q) {
			...
			$.ajax({
				url: base_url + 'admin/kategori_pelayanan_publik/prosesHapus',
				method: "POST",
				dataType: 'JSON',
				data: {
					id: id_hapus
		...
		$(document).on('click', '.update', function() {
			...
			$.ajax({
				url: base_url + 'admin/kategori_pelayanan_publik/get_id',
				method: "POST",
				data: {
					id: get_id
				},
		...
		$(document).on('submit', '#submit_form_tambah', function(event) {
			...
			if (kategori == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/kategori_pelayanan_publik/prosesTambah',
					method: "POST",
		...
		$(document).on('submit', '#submit_form_ubah', function(event) {
			...
			if (kategori == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/kategori_pelayanan_publik/prosesUbah',
					method: "POST",
					data: new FormData(this),
					dataType: 'JSON',
					contentType: false,
		...
		function Load_Data() {
			table = $('#myTable').DataTable({
				...
				pageLength: 10,
				autoWidth: false,
				ajax: base_url + 'admin/kategori_pelayanan_publik/get_data',
				columns: [{
						'data': (d) => {
							return d.no;
						}
