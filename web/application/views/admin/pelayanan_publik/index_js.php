<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		...
		$(document).on('click', '#konfirmasi_hapus', function(q) {
			...
			$.ajax({
				url: base_url + 'admin/pelayanan_publik/prosesHapus',
				method: "POST",
				dataType: 'JSON',
				data: {
					id: id_hapus
		...
		$(document).on('click', '.update', function() {
			...
			$.ajax({
				url: base_url + 'admin/pelayanan_publik/get_id',
				method: "POST",
				data: {
					id: get_id
				},
		...
		$.ajax({
			url: base_url + 'admin/pelayanan_publik/select_kategori',
			type: 'POST',
			dataType: 'JSON',
			success: function(data) {

				var html = "<option value=''>--- Silakan Pilih Kategori pelayanan_publik ---</option>";
		...
		$(document).on('submit', '#submit_form_tambah', function(event) {
			...
			/*if (extension != '') {
				$('.validasi').text('');
				if (jQuery.inArray(extension, ['pdf', 'csv', 'gif', 'png', 'jpg', 'jpeg']) == -1) {
					Swal.fire({
						icon: 'warning',
						title: 'Warning!',
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/pelayanan_publik/prosesTambah',
					method: "POST",
		...
		$(document).on('submit', '#submit_form_ubah', function(event) {
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/pelayanan_publik/prosesUbah',
					method: "POST",
					data: new FormData(this),
					dataType: 'JSON',
					contentType: false,
		...
		function Load_Data() {
			table = $('#myTable').DataTable({
				...
				pageLength: 10,
				autoWidth: false,
				ajax: base_url + 'admin/pelayanan_publik/get_data',
				columns: [{
						'data': (d) => {
							return d.no;
						}
					...
					},
					{
						'data': (d) => {
							...
							if (d.file == null || d.file == "") {
								...
								btn_link += `<a href="` + d.file + `" target="_blank" class="btn btn-sm btn-primary"> Lihat File</a> `;
							}
							return btn_link;
						}
					},
		...
		function file_preview(input, id_name) {
			if (input.files && input.files[0]) {
				...
				reader.onload = function(e) {
					//` + e.target.result + ` isi load file 
					$('#' + id_name + '_preview').remove();
					$('#' + id_name + '_preview_container').html(
						`<div style="border: 1px solid #ccc; border-style: dashed; padding: 5px;" class="text-center">
						<img src="https://icons.iconarchive.com/icons/graphicloads/long-shadow-documents/256/document-tick-icon.png" 
