<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		...
		$(document).on('click', '#konfirmasi_hapus', function(q) {
			...
			$.ajax({
				url: base_url + 'admin/ppid/prosesHapus',
				method: "POST",
				dataType: 'JSON',
				data: {
					id: id_hapus
		...
		$(document).on('click', '.update', function() {
			...
			$.ajax({
				url: base_url + 'admin/ppid/get_id',
				method: "POST",
				data: {
					id: get_id
				},
		...
		$.ajax({
			url: base_url + 'admin/ppid/select_kategori',
			type: 'POST',
			dataType: 'JSON',
			success: function(data) {

				var html = "<option>--- Silakan Pilih Kategori PPID ---</option>";
		...
		$(document).on('submit', '#submit_form_tambah', function(event) {
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/ppid/prosesTambah',
					method: "POST",
		...
		$(document).on('submit', '#submit_form_ubah', function(event) {
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'admin/ppid/prosesUbah',
					method: "POST",
					data: new FormData(this),
					dataType: 'JSON',
					contentType: false,
		...
		function Load_Data() {
			table = $('#myTable').DataTable({
				...
				pageLength: 10,
				autoWidth: false,
				ajax: base_url + 'admin/ppid/get_data',
				columns: [{
						'data': (d) => {
							return d.no;
						}
