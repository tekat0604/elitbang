...
$(document).ready(function(){ 
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/home/grid_dua/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/home/grid_dua/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/home/grid_dua/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/home/grid_dua/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/home/grid_dua/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},  
				{'data': (d)=>{
					...
					if(d.image==null || d.image==""){ 
						...
						img += '<img src="'+base_url+'uploads/grid_home/small/'+d.image+'" style="width: 80px;">';  
					}
					return img;
				}},
				{'data': 'judul'},
