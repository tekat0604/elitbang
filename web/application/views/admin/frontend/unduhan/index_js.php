...
$(document).ready(function(){ 
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/unduhan/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/unduhan/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
			success 	: function(data){
				var src_file = base_url+'uploads/menu/'+data.konten;  
					...
					if(data.konten!=''){  
						$('#ubah_file_preview_container').html(
							`<div style="border: 1px solid #ccc; border-style: dashed; padding: 5px;">
								...
								style="width: 50px; margin-right: 5px; "/><br>`+data.konten+`
							</div>
							<div>
							<a href="`+src_file+`" target="_blank"> `+data.konten+` </a>
							</div>
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(judul==""){ 
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/unduhan/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/unduhan/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/unduhan/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
				...
				{'data': (d)=>{
					var unduhan = '';  
					if(d.konten==null || d.konten==""){ 
						unduhan += "";  
					}else{
						unduhan += '<a href="'+base_url+'uploads/menu/'+d.konten+'" target="_blank"> <i class="fa fa-download"></i> Download </a>';  
					}
					return unduhan;
				}},
				{'data': (d)=>{
