...
$(document).ready(function(){
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/agenda_kegiatan/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/agenda_kegiatan/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/agenda_kegiatan/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/agenda_kegiatan/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/agenda_kegiatan/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
