...
class Unduhan extends MY_Controller {
    private $base           = 'admin';
    private $id_menu_utama  = 6;
    private $menu           = 'unduhan';
    ...
    public function prosesTambah(){ 
        if($_FILES['file']){ 
            $config['allowed_types']    = 'jpg|png|jpeg|gif|pdf|doc|docx|xls|xlsx|ppt|txt';
    ...
    public function prosesUbah(){
        ...
        $config = array(
            ...
            'allowed_types' => 'jpg|png|jpeg|gif|pdf|doc|docx|xls|xlsx|ppt|txt'
        );
        ...
        if($_FILES['file'] != ''){
            if(!$this->upload->do_upload('file')){
                if($kosongkan_file=="1"){
                    if($data_menu['konten']!=''){
                        unlink('./uploads/menu/'.$data_menu['konten']); 
                    }
                    $file_name = "";
                }else{
                    $file_name = $data_menu['konten'];
                }
            }else{
                if($data_menu['konten']!=''){
                    unlink('./uploads/menu/'.$data_menu['konten']); 
