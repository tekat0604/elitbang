...
class Api extends CI_Controller {
    ...
    {
        $res['data'] = $this->db->where('id_user',$this->session->userdata('id'))->get('tabel_api')->result_array();
        echo json_encode($res);
    }

    public function hapus_permohonan_api()
    {
        $cek = $this->db->where('id_api',$this->input->post('id_api'))->get('tabel_api')->row_array();
        if($cek === null)
        {
            $res['status'] = 'error';
        }
        ...
        {
            if($cek['id_user'] == $this->session->userdata('id'))
            {
