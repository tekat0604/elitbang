...
class Frontend extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            //'isi'                 => "home/index",
            //'locations'             => $arr_loc,
            'extra_js'              => "$this->base/home/index_js",
        ];
    ...
    public function unit_kerja()
    {
        ...
        $data = [
            'extra_css'         => "$this->base/unit_kerja/index_css",
            'extra_js'          => "$this->base/unit_kerja/index_js",
            'profil_pejabat'    => $profil_pejabat,
            'profil_pegawai'    => $profil_pegawai,
        ];

        $this->template->content_frontend("$this->base/unit_kerja/index", $data);
    }
    ...
    public function get_data()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->_sendPusher('load-notification-tables', ['load' => true]);
        }
    }

    public function get_filter_kategori()
    {
        $this->session->unset_userdata('id_pengaduan');
        $arr = [];
        for ($i = 1; $i <= 5; $i++) {
    ...
    {
        $get_data = $this->getDataApi();
        $json = [];
        foreach ($get_data[0]->result as $item) {
            ...
            $properties = [
                'type' => 'Koordinat data pengaduan',
                'content' => $item->content,
                'tgl_sug' => $item->tgl_sug,
                'jam_sug' => $item->jam_sug,
                'title' => $item->title,
                'status' => $item->status,
                'via' => $item->via,
                'name' => $item->name,
        ...
        $this->output
            ...
            ->set_output(json_encode($geoJson, JSON_NUMERIC_CHECK));
    }
}
