<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		...
		$(document).on('click', '#konfirmasi_hapus', function(q) {
			...
			$.ajax({
				url: base_url + 'operator/data_relawan/prosesHapus',
				method: "POST",
				dataType: 'JSON',					
				data: {
					id: id_hapus
		...
		$(document).on('click', '.update', function() {
			...
			$.ajax({
				url: base_url + 'operator/data_relawan/get_id',
				method: "POST",
				data: {
					id: get_id
				},
		...
		$(document).on('submit', '#submit_form_tambah', function(event) {
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'operator/data_relawan/prosesTambah',
					method: "POST",
		...
		$(document).on('submit', '#submit_form_ubah', function(event) {
			...
			if (judul == "") {
				...
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'operator/data_relawan/prosesUbah',
					method: "POST",
					data: new FormData(this),
					dataType: 'JSON',
					contentType: false,
		...
		function Load_Data() {
			table = $('#myTable').DataTable({
				...
				autoWidth: false,
				ajax: {
				'url' :base_url + 'operator/data_relawan/get_data',
				'dataSrc' : 'data',
				} ,
				columns: [
					{
