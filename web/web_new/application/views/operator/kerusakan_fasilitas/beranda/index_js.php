<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		...
		$(document).on('click', '#konfirmasi_hapus', function(q) {
			...
			$.ajax({
				url: base_url + 'operator/kerusakan_fasilitas/prosesHapus',
				method: "POST",
				dataType: 'JSON',					
				data: {
					id: id_hapus
		...
		$(document).on('click', '.update', function() {
			...
			$.ajax({
				url: base_url + 'operator/kerusakan_fasilitas/get_id',
				method: "POST",
				data: {
					id: get_id
				},
				...
				success: function(data) {			
					...
					$("#ubah_jumlah_unit").val(data.jumlah_unit);
					$("#ubah_kerugian_biaya").val(data.kerugian_biaya);
		...
		$(document).on('submit', '#submit_form_tambah', function(event) {
			...
			var judul = $('#tambah_jumlah_unit').val();
			if (judul == "") {
				$('.validasi').text('');
				$('#error_jumlah_unit').html('<p class="text-danger"> <i class="fa fa-warning"></i> Judul tidak boleh kosong</p>');
				$('#tambah_jumlah_unit').focus();
			} else {
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'operator/kerusakan_fasilitas/prosesTambah',
					method: "POST",
		...
		$(document).on('submit', '#submit_form_ubah', function(event) {
			event.preventDefault();
			var judul = $('#ubah_jumlah_unit').val();
			if (judul == "") {
				$('.validasi').text('');
				$('#error_ubah_jumlah_unit').html('<p class="text-danger"> <i class="fa fa-warning"></i> Judul tidak boleh kosong</p>');
				$('#ubah_jumlah_unit').focus();
			} else {
				$('.validasi').text('');
				$.ajax({
					url: base_url + 'operator/kerusakan_fasilitas/prosesUbah',
					method: "POST",
					data: new FormData(this),
					dataType: 'JSON',
					contentType: false,
		...
		function Load_Data() {
			table = $('#myTable').DataTable({
				...
				autoWidth: false,
				ajax: {
				'url' :base_url + 'operator/kerusakan_fasilitas/get_data',
				'dataSrc' : 'data',
				} ,
				columns: [
					{
					...
					},
					{
						'data': (d) => {
							var btn = '';
							btn += '<button type="button" class="btn btn-secondary mr-5 mb-5 update" id="' + d.id_kerusakan_fasilitas + '">';
							btn += '<i class="fa fa-edit"></i> Ubah </button> ';
							btn += '<button type="button" class="btn btn-secondary mr-5 mb-5 hapus"';
							btn += 'data-toggle="modal" data-id="' + d.id_kerusakan_fasilitas + '" data-target="#konfirmasi_hapus_modal">';
