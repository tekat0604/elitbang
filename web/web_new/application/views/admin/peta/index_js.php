...
$(document).ready(function(){
    $.ajax({
        url:'<?php echo base_url()?>admin/peta/index',
        type:'GET',
        success: function(data){
            console.table(data);
           $('#result_link_content').html($(data).find('#link_content').html());
        }
    }); 
    defaultForm();
    table = $('#mydata').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('admin/peta/daftar_layer_peta')?>",
            "type": "POST",
            "data": function ( data ) {
    ...
    $('#btn_tambah_layer').click(function(){
        $.ajax({
            url: '<?=base_url()?>admin/peta/get_grup_layer',
            type: 'GET',
            dataType: 'JSON'
        })
        .then(v=>{
        ...
        $.ajax({
            url: '<?=base_url()?>admin/peta/get_jenis_peta',
            type: 'GET',
            dataType: 'JSON'
        })
        .then(v=>{
    ...
    $('#btn_simpan_grup_layer').click(function(){
		...
		$.ajax({
			url: '<?=base_url()?>admin/peta/simpan_grup_layer',
			type: 'POST',
			data: fd,
			dataType: 'JSON',
			processData: false,
    ...
    $('#btn_simpan_jenis_peta').click(function(){
		...
		$.ajax({
			url: '<?=base_url()?>admin/peta/simpan_jenis_peta',
			type: 'POST',
			data: fd,
			dataType: 'JSON',
			processData: false,
...
$('#tambah_layer').submit(function(e){
    e.preventDefault();
    $.ajax({
        url:'<?php echo base_url();?>admin/peta/simpan_layer',
        type:"post",
...
$('#mydata').on('click','.btn_kelola',function(){
    ...
    window.location.replace('<?= base_url('admin/peta/kelola/');?>'+id);
});

$('#mydata').on('click','.btn_data',function(){
    var id = $(this).attr('data');
    window.location.replace('<?= base_url('admin/peta/data_peta/');?>'+id);
});

$('#mydata').on('click','.btn_group',function(){
    var id = $(this).attr('data');
    window.location.replace('<?= base_url('admin/peta/grup_atribut/');?>'+id);
});

function hapus_semua_data(id){
    $.ajax({
    type : "POST",
    url  : "<?php echo base_url('admin/peta/hapus_semua_data_layer')?>",
    dataType : "JSON",
...
function hapus(id){
    $.ajax({
    type : "POST",
    url  : "<?php echo base_url('admin/peta/hapus_layer')?>",
    dataType : "JSON",
            data : {id: id, '<?php echo $this->security->get_csrf_token_name(); ?>':'<?php echo $this->security->get_csrf_hash(); ?>'},
            success: function(data){
                defaultForm();
...
function simpan_item_grup_layer(id,nama)
{
	...
	$.ajax({
		url: '<?=base_url()?>admin/peta/edit_grup_layer',
		type: 'POST',
		data: fd,
		dataType: 'JSON',
		processData: false,
...
function hapus_item_grup_layer(id)
{
	Swal.fire({
	    	...
	    	if (result.value) {
				...
				$.ajax({
					url: '<?=base_url()?>admin/peta/hapus_grup_layer',
					type : 'POST',
					dataType : 'JSON',
...
function simpan_item_jenis_peta(id,nama)
{
	...
	$.ajax({
		url: '<?=base_url()?>admin/peta/edit_jenis_peta',
		type: 'POST',
		data: fd,
		dataType: 'JSON',
		processData: false,
...
function hapus_item_jenis_peta(id)
{
	Swal.fire({
	    	...
	    	if (result.value) {
				...
				$.ajax({
					url: '<?=base_url()?>admin/peta/hapus_jenis_peta',
					type : 'POST',
					dataType : 'JSON',
