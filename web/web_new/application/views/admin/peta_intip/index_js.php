...
$(document).ready(function(){
    ...
    table = $('#mydata').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('admin/peta_intip/daftar_layer_peta')?>",
            "type": "POST",
            "data": function ( data ) {
...
$('#mydata').on('click','.btn_kelola',function(){
    ...
    window.location.replace('<?= base_url('admin/peta_intip/kelola/');?>'+id);
});

$('#mydata').on('click','.btn_data',function(){
    var id = $(this).attr('data');
    window.location.replace('<?= base_url('admin/peta_intip/data_peta/');?>'+id);
});

$('#mydata').on('click','.btn_group',function(){
    var id = $(this).attr('data');
    window.location.replace('<?= base_url('admin/peta_intip/grup_atribut/');?>'+id);
});

function hapus_semua_data(id){
    $.ajax({
    type : "POST",
    url  : "<?php echo base_url('admin/peta_intip/hapus_semua_data_layer')?>",
    dataType : "JSON",
...
function hapus(id){
    $.ajax({
    type : "POST",
    url  : "<?php echo base_url('admin/peta_intip/hapus_layer')?>",
    dataType : "JSON",
            data : {id: id, '<?php echo $this->security->get_csrf_token_name(); ?>':'<?php echo $this->security->get_csrf_hash(); ?>'},
            success: function(data){
                defaultForm();
