...
<script type="text/javascript">
    // Upload Foto
      var dropzone= new Dropzone(".dropzone",{
        url: "<?php echo base_url()?>admin/peta_intip/upload_foto/",
    ...
    function tampil_foto(id_collection){
        $.ajax({
          type: 'GET',
          url: '<?= base_url();?>admin/peta_intip/ambil_foto/',
    ...
    function show_modal_hapus(id, id_collection){
      Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                  type: 'POST',
                  url: '<?php echo base_url()?>admin/peta_intip/do_delete/',
...
$(document).ready(function(){
    ...
    function tampil_data(){
        ...
        $.ajax({
            type  : 'post',
            url   : '<?php echo base_url()?>admin/peta_intip/get_data_layer/<?=$this->uri->segment(4)?>',
            async : false,
            dataType : 'json',
            success : function(data){
                var html = '';
                ...
                for(i=0; i<data['data'].length; i++){
                    ...
                    html += '<td style="text-align: center;">'+
                                '<a href="<?= base_url();?>admin/peta_intip/edit_data_peta/'+data['data'][i].id_layer+'/'+data['data'][i].tipe_layer+'/'+data['data'][i].id_collection+'" class="btn btn-sm btn-warning mb-10" data="'+data['data'][i].id_layer+'" title="Edit Data"><i class="fa fa-pencil"></i></a> '+
                                ' '+
                                ...
                                ' '+
                                '<a href="<?= base_url();?>admin/peta_intip/diskripsi/<?= $id;?>/'+data['data'][i].id_collection+'" class="btn btn-sm btn-success mb-10" data="'+data['data'][i].id_layer+'" title="Deskripsi Halaman Detail"><i class="fa fa-file"></i></a> '+
                                ' '+
    ...
    $('#import_geojson').on('submit', function(e){
        ...
        e.preventDefault();
        $.ajax({
            url: "<?=base_url()?>admin/peta_intip/import_data_peta",
            type: "POST",
    ...
    $('#btn_import_template').on('click', function(e){
        $.ajax({
            url: "<?=base_url()?>admin/peta_intip/import_template",
            type: "POST",
...
$( ".btn-simpan" ).click(function() {
    tipe = $(".tipe").val();
    if(tipe != ""){
        window.location.replace("<?= base_url('admin/peta_intip/tambah_data_peta/').$this->uri->segment(4); ?>/"+tipe);
    }else{
        Swal.fire({
            title : 'Perhatian!',
...
function hapus_data(id_collection){
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type  : 'POST',
                url   : '<?php echo base_url()?>admin/peta_intip/hapus_data_layer',
                async : false,
                dataType : 'json',
                data  : {id_collection : id_collection},
                success : function(data){
