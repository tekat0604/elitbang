...
$(document).ready(function(){
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/galeri/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/galeri/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/galeri/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/galeri/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/galeri/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
				...
				{'data': (d)=>{
					var btn = '';  
					btn += '<a href="'+base_url+'admin/frontend/detail_galeri/index/'+d.id+'" class="btn btn-secondary mr-5 mb-5" >';  
					btn += '<i class="fa fa-plus"></i> Tambah Data </a> '; 
