...
$(document).ready(function(){
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/profil/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/profil/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(extension 	!= ''){
			...
			if(jQuery.inArray(extension, ['gif','png','jpg','jpeg']) == -1){ 
		        ...
		        $('#tambah_image_preview_container').html(''); 
		        $('#tambah_label_image').text('Silahkan pilih file...'); 
		        $('#tambah_image').val(''); 
				return false;
			}
		}
		if(judul==""){
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/profil/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/profil/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/profil/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
				...
				{'data': (d)=>{
					var btn = '';  
					btn += '<button type="button" class="btn btn-secondary update" id="'+d.id+'">';  
					btn += '<i class="fa fa-edit"></i> Ubah </button> '; 
					btn += '<button type="button" class="btn btn-secondary hapus"';  
