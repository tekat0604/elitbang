...
$(document).ready(function(){
	...
	var id_menu = "<?php echo $this->uri->segment(5);?>";   
	Load_Data();
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/detail_galeri/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$('#tambah_data').click(function(){ 
		$("#tambah_judul").focus();
		$("#tambah_jenis").attr("checked",false); 
		$('#tambah_link_youtube').attr('type', 'hidden');
		setTimeout(function(){
			$('#tambah_image_preview_container').html("");  
			$('#tambah_image_label').text("");  
			$("#tambah_id_menu").val(id_menu);
		}, 500);  
	...
	$('#tambah_jenis').change(function() {
        if($(this).is(":checked")) {
			$('#tambah_link_youtube').attr('type', 'text');
        }else{
			$('#tambah_link_youtube').attr('type', 'hidden');
		}
    });
    $('#ubah_jenis').change(function() {
        if($(this).is(":checked")) {
			$('#ubah_link_youtube').attr('type', 'text');
        }else{
			$('#ubah_link_youtube').attr('type', 'hidden');
		}
    });
	
	$(document).on('click','.update', function(){
		$("#ubah_jenis").attr("checked",true); 
		$('#ubah_link_youtube').attr('type', 'text');
		$('#ubah_image_label').text("");
		var get_id = $(this).attr("id"); 
		$.ajax({ 
			url			: base_url+'admin/frontend/detail_galeri/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
			success 	: function(data){
				var src_img = base_url+'uploads/sub_menu/small/'+data.image; 
				//alert(data.id);
				...
				setTimeout(function(){ 
					...
					if(data.jenis==1){
						...
						$("#ubah_jenis").attr("checked",false);
						$('#ubah_link_youtube').attr('type', 'hidden');	
					}
					$('#ubah_link_youtube').val(data.link); 
				}, 500);
			}
		});
	});
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/detail_galeri/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		...
		if(judul==""){
			...
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/detail_galeri/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){ 
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/detail_galeri/get_data/'+id_menu, 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
				...
				{'data': (d)=>{
					...
					if(d.image==null || d.image==""){ 
						...
						img += '<img src="'+base_url+'uploads/sub_menu/small/'+d.image+'" style="width: 100px;">';  
					}
					return img;
				}},
				{'data': (d)=>{
					var label_jenis = '';  
					if(d.jenis!='1'){ 
						label_jenis += '<span class="badge badge-success" style="width: 100px;"> <i class="si si-picture"></i> Foto </span>';  
					}else{ 
						label_jenis += '<span class="badge badge-danger" style="width: 100px;"> <i class="si si-social-youtube"></i> Video </span>';  
					}
					return label_jenis;
				}},
				{'data': (d)=>{
