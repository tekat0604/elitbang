...
$(document).ready(function(){ 
	...
	$(document).on('click','#konfirmasi_hapus', function(q){
		...
		$.ajax({
			url			: base_url+'admin/frontend/home/pesan_singkat/prosesHapus', 
			method 		: "POST", 
			dataType 	: 'JSON',
			data 		: {id:id_hapus}, 
	...
	$(document).on('click','.update', function(){
		...
		$.ajax({ 
			url			: base_url+'admin/frontend/home/pesan_singkat/get_id', 
			method 		: "POST",
			data 		: {id:get_id}, 
			dataType 	: "json",
			success 	: function(data){
				var src_img = base_url+'uploads/grid_home/small/'+data.image;  
				$('#formModalUbah').modal('show'); 
				$("#ubah_id").val(get_id);  
				setTimeout(function(){ 
					$("#ubah_konten").val(data.konten); 
	...
	$(document).on('submit','#submit_form_tambah', function(event){
		...
		var konten 		= $('#tambah_konten').val(); 
		if(konten==""){
			$('.validasi').text(''); 
			$('#error_tambah_konten').html('<p class="text-danger"> <i class="fa fa-warning"></i> Konten tidak boleh kosong</p>'); 
			$('#tambah_konten').focus(); 
		}else{
			$('.validasi').text(''); 
			$.ajax({ 
				url 		: base_url+'admin/frontend/home/pesan_singkat/prosesTambah', 
				method 		: "POST",
	...
	$(document).on('submit','#submit_form_ubah', function(event){
		event.preventDefault(); 
		var konten 		= $('#ubah_konten').val();   
		if(konten==""){
			$('.validasi').text(''); 
			$('#error_ubah_konten').html('<p class="text-danger"> <i class="fa fa-warning"></i> Konten tidak boleh kosong</p>'); 
			$('#ubah_konten').focus(); 
		}else{
			$('.validasi').text(''); 
				$.ajax({ 
				url 		: base_url+'admin/frontend/home/pesan_singkat/prosesUbah', 
				method 		: "POST",
				data 		: new FormData(this),
				dataType 	: 'JSON',
				contentType : false,
	...
	function Load_Data(){
		table = $('#myTable').DataTable({
			...
			pageLength	: 10, 
			autoWidth	: false, 
			ajax		: base_url+'admin/frontend/home/pesan_singkat/get_data', 
			columns: [
				{'data': (d)=>{
					return d.no;
				}},
