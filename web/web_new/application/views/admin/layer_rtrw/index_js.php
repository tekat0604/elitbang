...
$(document).ready(function(){
	...
	$('#form_tambah_layer').submit(function(e){
		...
		if(mode == 'add')
		{
			submit_url = '<?=base_url()?>admin/layer_rtrw/simpan_layer';
		}
		else if(mode == 'edit')
		{
			submit_url = '<?=base_url()?>admin/layer_rtrw/edit_layer';
		}
		...
		if(cek_nama_layer){
			...
			.done((res)=>{
				...
				if(res.status == 'success')
				{
					...
					window.location.replace('<?=base_url()?>admin/layer_rtrw');
				}
				else
				{
					Swal.fire({
	...
	$('#btn_simpan_grup_layer').click(function(){
		...
		$.ajax({
			url: '<?=base_url()?>admin/layer_rtrw/simpan_grup_layer',
			type: 'POST',
			data: fd,
			dataType: 'JSON',
			processData: false,
	...
	$('input[name="nama_layer"]').blur(function(){
		if($('input[name="nama_layer"]').val() != '')
		{
			...
			$.ajax({
				url: '<?=base_url()?>admin/layer_rtrw/cek_nama_layer',
				type: 'POST',
				dataType: 'JSON',
...
function tampil_layer(){
	...
	//get grup layer
	$.ajax({
		url: '<?=base_url()?>admin/layer_rtrw/get_grup_layer',
		type: 'GET',
		dataType: 'JSON'
	})
	.done((res)=>{
	...
	//get opd
	$.ajax({
		url: '<?=base_url()?>admin/layer_rtrw/get_opd',
		type: 'GET',
		dataType: 'JSON'
	})
	.done((res)=>{
...
function simpan_item_grup_layer(id,nama)
{
	...
	$.ajax({
		url: '<?=base_url()?>admin/layer_rtrw/edit_grup_layer',
		type: 'POST',
		data: fd,
		dataType: 'JSON',
		processData: false,
...
function hapus_item_grup_layer(id)
{
	Swal.fire({
	    	...
	    	if (result.value) {
				...
				$.ajax({
					url: '<?=base_url()?>admin/layer_rtrw/hapus_grup_layer',
					type : 'POST',
					dataType : 'JSON',
...
function ganti_status(id){
	...
	$.ajax({
		url: '<?=base_url()?>admin/layer_rtrw/ganti_status',
		type: 'POST',
		dataType: 'JSON',
...
function edit_layer(id){
	...
	//get layer
	$.ajax({
		url: '<?=base_url()?>admin/layer_rtrw/get_layer_by_id',
		type: 'POST',
		dataType: 'JSON',
		data: {id: id}
	})
...
function hapus_layer(id,slug){
	Swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '<?=base_url()?>admin/layer_rtrw/hapus_layer',
				type : 'POST',
				dataType : 'JSON',
...
function download_layer(id,slug){
	...
	window.open('<?=base_url()?>admin/layer_rtrw/download_geojson/'+slug, '_blank');
...
function list_layer(){
	$('#table_layer').dataTable({
		...
		autoWidth: false,
		ajax: "<?=base_url()?>admin/layer_rtrw/get_layer",
		columns: [
			{'data': 'nama_layer'},
