...
<head>
    ...
    <style>
        ...
        .letter-title .title { font-weight: bold; text-decoration: underline; font-size: 12pt; margin-bottom: 2px; }
    ...
    <div class="content">
        ...
        <table class="identity-table">
            <tr>
                ...
                <td style="width: 77%;"><b>{{ $pejabat_brida->nama_kepala_instansi }}</b></td>
            </tr>
            <tr>
                <td>Jabatan</td>
        ...
        <table class="identity-table">
            <tr>
                <td style="width: 20%;">Nama</td>
                <td style="width: 3%; text-align: center;">:</td>
                <td style="width: 77%;">{{ $permohonan->pemohon->nama_lengkap }}</td>
            </tr>
        ...
        @php
            ...
            $diffBulan = $start->diffInMonths($end);
            $durasiText = $diffBulan > 0 ? $diffBulan . ' bulan' : $start->diffInDays($end) . ' hari';

            $jenjang = strtoupper($permohonan->jenjang_pendidikan);
            $penyusunan = 'karya tulis ilmiah';
            if (in_array($jenjang, ['S1', 'D4'])) {
                $penyusunan = 'skripsi';
            } elseif ($jenjang == 'S2') {
                $penyusunan = 'tesis';
            } elseif ($jenjang == 'S3') {
                $penyusunan = 'disertasi';
            } elseif (in_array($jenjang, ['D3', 'SMA', 'SMP'])) {
                $penyusunan = 'tugas akhir';
            }
        @endphp

        <p class="statement">
            Telah selesai melakukan penelitian di Kota Surakarta selama {{ $durasiText }}, 
            terhitung mulai tanggal {{ $start->locale('id')->isoFormat('D MMMM Y') }} sampai dengan {{ $end->locale('id')->isoFormat('D MMMM Y') }} 
            untuk memperoleh data dalam rangka penyusunan {{ $penyusunan }} yang berjudul :
        </p>
